...
<div class="main_wrapper">
	...
	<input type="submit" name="search" value="search" />
		...
		<div class="content_wrapper">
			...
			<li><a href="addscientist.php">+ Add Sceintist</a></li>
			</ul>
			</div>
			<?php
      ...
      <tbody>
        <?php
          while( $row = mysqli_fetch_array( $selected ) ){
            ...
            echo
            "<tr>
              <td>{$row['scientist_id']}</td>
              <td>{$row['scientist_name']}</td>
              <td>{$row['scientist_email']}</td>
			   <td>{$row['scientist_address']}</td>
              <td>{$row['scientist_phone']}</td>
          </tr>\n";
          }
        ?>
      </tbody>
