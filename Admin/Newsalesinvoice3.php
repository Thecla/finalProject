...
if(isset($_POST['processpayment'])){
	...
	detail_id='{$id}',
	cashier_name='{$_POST['cashier_name'][$i]}',
	patient_name='{$_POST['patient_name'][$i]}',
	patient_id='{$_POST['patient_id'][$i]}',	
	items_purchased='{$_POST['Product_name'][$i]}',	
...
<!DOCTYPE html>
<?php
require_once("Functions/Newfunctions.php");
...
<div class="main_wrapper">
	...
	<input type="submit" name="search" value="search" />
		...
		<div class="content_wrapper">
			...
			<form method="POST" action="Newsalesinvoice3.php">
			...
			<br />
			<form action ="Newsalesinvoice3.php" method="post" enctype="multipart/form-data">
...
foreach($_SESSION as $name => $value){
	...
	<td><input type='text' name='qty' value="<?php echo $value; ?>"/></td>
			...
			<td><a href='Newsalesinvoice3.php?remove=$product_id'>[-]</a> <a href='Newsalesinvoice3.php?add=$product_id'>[+]</a> <a href='Newsalesinvoice3.php?delete=$product_id'>[Delete]</a> </td>  
			</tr> \n";?>
		<?php	}}}}
			?>
			<?php
...
$product_id=$_GET['add'];
	...
	header("Location:Newsalesinvoice3.php");
}
...
<?php
	if(isset($_POST['submitDetails'])){
		...
		// echo '<script>alert("'.$pt1.'");</script>';
		// echo '<script>alert("'.$pt2.'");</script>';
		// echo '<script>alert("'.$pt3.'");</script>';
		// echo '<script>alert("'.$pt4.'");</script>';
	}
?>
